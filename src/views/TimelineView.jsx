...
export default function TimelineView({ deals, onOpenDeal }) {
  // Multi-select stages — empty set means all shown
  ...
  // Filter: empty set = all stages shown; otherwise only selected stages
  const filtered = deals.filter(d => activeStages.size === 0 || activeStages.has(d.stage))
  ...
  return (
    <div style={{ padding: '24px', height: 'calc(100vh - 52px)', overflowY: 'auto', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', gap: '12px' }}>
        ...
        {/* Multi-select stage toggles */}
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', alignItems: 'center' }}>
          {activeStages.size > 0 && (
            <button
              onClick={() => setActiveStages(new Set())}
              style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontFamily: 'DM Mono, monospace', fontSize: '11px', padding: '4px 6px', letterSpacing: '0.04em' }}
            >
              Clear
            </button>
          )}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${active ? color.border : '#2a2a2a'}`,
                  borderRadius: '5px',
